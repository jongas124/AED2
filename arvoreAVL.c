...
struct Node* RightRotate(struct Node* y) {
    ...
    y->Altura = __max(Altura(y->Left), Altura(y->Right)) + 1;
    x->Altura = __max(Altura(x->Left), Altura(x->Right)) + 1;
...
struct Node* addNode(struct Node* raiz, char* nome, char* genero, char* plataforma, int ano) {
    ...
    raiz->Altura = 1 + __max(Altura(raiz->Left), Altura(raiz->Right));
...
void impressaoGenero(struct Node* raiz, char* genero) {
    if (raiz == NULL) {
        printf("Nao temos copia de jogos deste genero!");
        return;
    }    
    if(raiz->Left != NULL){
        impressaoGenero(raiz->Left, genero); // Busca na subárvore esquerda
    }
    if (strstr(raiz->genero, genero) != NULL) {
        // O nó atual tem o gênero buscado, imprime o nome do jogo
        printf("%s\n", raiz->nome);
    }
    if(raiz->Right != NULL){
        impressaoGenero(raiz->Right, genero); // Busca na subárvore direita
    }
...
void impressaoPlataforma(struct Node* raiz, char* plataforma) {    
    if (raiz == NULL) {
        printf("Nao temos copia de jogos nesta plataforma!");
        return;
    }

    
    if(raiz->Left != NULL){
        impressaoPlataforma(raiz->Left, plataforma); // Busca na subárvore esquerda
    }
    if (strcmp(raiz->plataforma, plataforma) == 0) {
        // O nó atual tem a plataforma buscada, imprime o nome do jogo
        printf("%s\n", raiz->nome);
    }
    if(raiz->Right != NULL){
        impressaoPlataforma(raiz->Right, plataforma); // Busca na subárvore direita
    }
}
...
void impressaoAno(struct Node* raiz, int ano) {
    if (raiz == NULL) {
        printf("Nao temos copia de jogos deste ano!");
        return;
    }

    
    if(raiz->Left != NULL){
        impressaoAno(raiz->Left, ano); // Busca na subárvore esquerda
    }
    if (raiz->anoLanc == ano) {
        // O nó atual tem o ano de lançamento buscado, imprime o nome do jogo
        printf("%s\n", raiz->nome);
    }
    if(raiz->Right != NULL){
        impressaoAno(raiz->Right, ano); // Busca na subárvore direita
    }
